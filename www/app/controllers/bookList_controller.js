cabApp.controller("bookController", ['$scope', '$http', '$rootScope', '$state', function ($scope, $http, $rootScope, $state) {
    ...
    $scope.deleteBook = function (m) {
        ...
        if (m.list) {
            ...
            $http({
                ...
                location.reload();
                if (response.status) {
                    $scope.book_delete = "cab deleted successfully";

                } else {
                    $scope.book_delete = "cab not deleted";
                }
            })
        }
